export type CodexMagicAction = "search-sources" | "check-accuracy" | "check-prose";
...
export function buildCodexMagicPrompt(
  ...
  if (action === "search-sources") {
    return [
      ...
      "",
      "Selected sources:",
      ...(input.sources?.length ? input.sources.map((path) => `- ${path}`) : ["- all indexed sources"]),
    ].join("\n");
  }
  ...
  if (action === "check-accuracy") {
    return [
      ...
      "",
      "Examine the selected manuscript sections against the project sources, notes, claims, and Codex notes.",
      ...
      "4. Suggested Codex notes or follow-up searches, if useful.",
      "",
      "Selected manuscript sections:",
      ...(input.sections?.length ? input.sections.map((path) => `- ${path}`) : ["- all manuscript sections"]),
    ].join("\n");
  }
